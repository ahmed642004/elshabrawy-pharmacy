...
export default function DeliveryStep({
  ...
  return (
    <div className="flex flex-col gap-4">
      <h2 className="m-0 font-headline text-xl font-extrabold text-neutral-900">{t("deliveryTitle")}</h2>
      ...
      <div className="flex flex-col gap-2.5">
            ...
            >
              ...
              <span>
                <span className="block font-headline text-sm font-bold text-neutral-900">
                  {a.isDefault ? t("defaultAddress") : t("addressLabel")} — {a.name}
                </span>
        ...
        >
          <PlusCircle className="h-[18px] w-[18px]" /> {t("addNewAddress")}
        </button>
      </div>
      ...
      {addingNew && (
        <div className="flex flex-col gap-3.5 rounded-[14px] border border-neutral-200 bg-white p-4.5">
          ...
          <div className="grid grid-cols-1 gap-3.5 sm:grid-cols-2">
            ...
            <div>
              <Input placeholder={t("phone")} value={form.phone} onChange={(e) => onFormChange("phone", e.target.value)} />
          ...
          <div>
            <Input placeholder={t("city")} value={form.city} onChange={(e) => onFormChange("city", e.target.value)} />
      ...
      {showEta && (
        <div className="flex items-center gap-2.5 rounded-[10px] border border-secondary-100 bg-secondary-50 px-3.5 py-3 text-[13.5px] text-neutral-700">
          <Truck className="h-[18px] w-[18px] shrink-0 text-secondary-600" />
          <span>{t.rich("eta", { strong: (chunks) => <strong>{chunks}</strong> })}</span>
        </div>
      )}
    </div>
  );
